...
export class FuelItem extends Item {
  ...
  consume(consumer: GameObject): void {
    if (!(consumer instanceof Spaceship)) return;

    consumer.fuel += this.fuelGain;
    consumer.fuel = Math.min(consumer.fuel, consumer.maxFuel);
  ...
  getElement(): SVGElement {
    ...
    packet.setAttribute("stroke", "red");
