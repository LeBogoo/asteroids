...
export class AsteroidManager implements Updateable {
  ...
  private spawnAsteroidInCone(): void {
    ...
    const distance = this.spawnDistance + Math.random() * 200;
    ...
    const asteroidAngle = Math.random() * 360;
    const asteroidSize = 20 + Math.random() * 40;

    const asteroid = new Asteroid(spawnPos, asteroidAngle, asteroidSize);
    asteroid.targetAngularVelocity = (Math.random() - 0.5) * 100;
